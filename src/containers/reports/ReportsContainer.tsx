...
import { useCashflow, useCashInOut, useStatusReport, useCalendar, useRefreshReports } from '@/hooks/useReports';
...
export default function ReportsContainer() {
  ...
  return (
    <>
      <PageHeader title="Reports" subtitle="Family cash flow, status snapshots, and premium calendars" />
        ...
        <CardContent>
          <Box display="flex" gap={2} alignItems="center">
            <TextField
              ...
              sx={{ width: 200 }}
            />
            <Button variant="contained" onClick={handleLoad} disabled={!inputCode}>Load</Button>
            {familyCode && (
              <Button
                variant="outlined"
      ...
      {tab === 0 && (
        <Card>
          <CardContent>
            <Typography variant="h6" fontWeight={600} mb={2}>Year-by-Year Cash Flow {familyCode && `— ${familyCode}`}</Typography>
            {cfLoading ? <LoadingState /> : !cashflow?.length ? (
      ...
      {tab === 2 && (
        <Card>
          <CardContent>
            <Typography variant="h6" fontWeight={600} mb={2}>Policy Status Snapshot {familyCode && `— ${familyCode}`}</Typography>
            {!status?.length ? (
              <EmptyState title="No data" message="Enter a family code and click Load." />
            ) : (
              <TableContainer>
                  ...
                  <TableBody>
                    {status.map((s) => (
                      <TableRow key={s.policyNo} hover>
                        ...
                        <TableCell align="right">{formatCurrency(s.bonus)}</TableCell>
                        <TableCell align="right">{formatCurrency(s.surrenderValue)}</TableCell>
      ...
      {tab === 3 && (
        <Card>
          <CardContent>
            <Typography variant="h6" fontWeight={600} mb={2}>12-Month Premium Calendar {familyCode && `— ${familyCode}`}</Typography>
            {!calendar?.length ? (
              <EmptyState title="No data" message="Enter a family code and click Load." />
            ) : (
                      ...
                      <CardContent sx={{ pb: '12px !important' }}>
                        ...
                        {entry.policies.map((p) => (
                          <Box key={p.policyNo} display="flex" justifyContent="space-between" py={0.5}>
                            <Typography variant="caption" sx={{ fontFamily: 'monospace' }}>{p.policyNo}</Typography>
